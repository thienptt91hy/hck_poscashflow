...
export function DashboardCharts({
  ...
  const pieData = [
    { name: dict.sales.cash, value: paymentMix.cash },
    { name: dict.sales.qrCard, value: paymentMix.qr },
    { name: dict.sales.bankTransfer, value: paymentMix.bank },
  ].filter((d) => d.value > 0);

  const totalPayment = pieData.reduce((s, d) => s + d.value, 0);

  const activeStores = stores.filter((s) =>
    areaData.some((row) => (row[s.id] as number) > 0),
  );

  return (
    <div className="space-y-4">
      {/* Row 1: Area chart + Payment donut */}
      ...
      {/* Row 2: Store breakdown bar chart */}
      {storeData.length > 0 && (
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-semibold text-zinc-700">
          ...
          <CardContent className="pt-0 pb-4">
            <div style={{ height: Math.max(storeData.length * 52, 80) }} className="w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart
                  data={storeData}
                  layout="vertical"
