  ...
  return (
    <div className="space-y-8 max-w-3xl">
          ...
          <table className="w-full text-sm">
            <thead className="bg-zinc-50 text-xs uppercase text-zinc-500">
              <tr>
                ...
                <th className="px-4 py-2.5 text-center">{dict.common.update}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-zinc-100">
              {(users ?? []).map((u) => {
                ...
                return (
                  <tr key={u.id} className="hover:bg-zinc-50">
                    <td className="px-4 py-2.5 text-zinc-600 text-xs">{u.email}</td>
      ...
      <p className="text-xs text-zinc-400">
        Để thêm user mới: Supabase Dashboard → Authentication → Users → Add user.
        Sau đó dùng bảng trên để gán role.
      </p>
    </div>
  );
}
